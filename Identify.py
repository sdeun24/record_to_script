...
def features(wav_file):
    ...
    Object = {'label' : 'test','feat' : feature}
...
def number_classes(train_dir):
    num = 0
    id_num = os.listdir(train_dir)
    for i in id_num:
        num = num + 1
    return num
...
def main(segment_path):
    ...
    embedding_dir = 'model/enroll_embeddings' # Where embeddings are saved
    test_dir = 'source/test/' # Where test features are saved
    
    # Settings
    use_cuda = True # Use cuda or not
    embedding_size = 128 # Dimension of speaker embeddings
    cp_num = 24 # Which checkpoint to use?
    n_classes = number_classes('source/train') # How many speakers in training data?
    #n_classes = 240
    ...
    spk_list = os.listdir(test_dir)
    test_path = features(segment_path)
    test_path = 'temp.p'
